  ...
  return (
    <section className="grid md:grid-cols-3 lg:grid-cols-5 gap-4 mt-2">
      {!notes.some((note) => note.archived) && (
        ...
        >
          <article
            className={`border bg-inherit p-3 shadow-md hover:scale-105 transition-all cursor-pointer h-full flex items-center justify-center`}
          >
            <svg
      ...
      {notes.length
        ? notes.map((note) => <CardItem key={note.id} {...note} />)
        : null}
    </section>
  );
}
